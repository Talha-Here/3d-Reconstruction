...
def plot_trajectory(fps):
    tracker = IMUTracker(sampling=fps)   
    data = receive_data('file')    # toggle data source between 'tcp' and 'file' here

    print('initializing...')
    init_list = tracker.initialize(data[0:30])  #data[5:30]

    print('--------')
    print('processing...')
    
    # EKF step
    a_nav, orix, oriy, oriz = tracker.attitudeTrack(data[0:], init_list)   #data[30:]
    ...
    euclidean_distance = distance.euclidean(last_position,first_position)
